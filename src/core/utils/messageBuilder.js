...
export function getFormattedTime(time = new Date()){
	let sendingTime = new Date(time.getUTCFullYear(), time.getUTCMonth(), time.getUTCDate(), time.getUTCHours(), time.getUTCMinutes(), time.getUTCSeconds(), time.getUTCMilliseconds());
	return (1900 + sendingTime.getYear()) + pad('00', sendingTime.getMonth() + 1, true) + pad('00', sendingTime.getDate(), true) + '-' + pad('00', sendingTime.getHours(), true) + ':' + pad('00', sendingTime.getMinutes(), true) + ':' + pad('00', sendingTime.getSeconds(), true) + '.' + pad('000', sendingTime.getMilliseconds(), false);
}

export function createFIXstring(message, msgType, client, transforms) {
	let bodyArray = [];

	let sendingTimeString = getFormattedTime();
	...
	let adminFields = ['MsgType', 'MsgSeqNum', 'TargetCompID', 'SenderCompID', 'SendingTime', 'BeginString', 'BodyLength'];
